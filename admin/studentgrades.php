...
$cur = $grade->single_grades($gradeId);
	...
	if (isset($_POST['savegrades'])) {
		$finalave = floatval($_POST['finalave']);
	
		// Perform grade calculation logic
		if ($finalave >= 3.1) {
			$remarks = "Failed";
		} else {
			$remarks = "Passed";
		...
		$grade->FIRST       = floatval($_POST['first']);
		$grade->SECOND      = floatval($_POST['second']);
		$grade->THIRD       = floatval($_POST['third']);
		$grade->FOURTH      = floatval($_POST['fourth']);
		$grade->AVE         = $finalave;
